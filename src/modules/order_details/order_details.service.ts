...
export class OrderDetailsService {
  ...
  async getTopSellingProducts(
    ...
    try {
      ...
      const mergedResults = rawResults.reduce((acc, curr) => {
        ...
        if (existing) {
          ...
          acc.push({
            productSampleName: curr.productSampleName,
            totalSold: parseFloat(curr.totalSold),
            totalInventQuantity: 0, // Sẽ tính sau
            unitName: curr.unitName,
            minConversionRate: parseFloat(curr.minConversionRate || '0'), // Lưu để so sánh
            productUnitId: curr.productUnitId, // Giữ productUnitId để tính tồn kho
          });
        }
      ...
      // Duyệt qua các `batches` để tính `totalInventQuantity`
      for (const result of mergedResults) {
        const productUnitId = result.productUnitId;

        const batches =
          await this.batchService.getBatchesByProductUnitId(productUnitId);

        result.totalInventQuantity = batches.reduce((total, batch) => {
          const conversionRate = batch.productUnit?.conversionRate || 1;
          return total + batch.inventQuantity * conversionRate;
        }, 0);

        // Xóa `productUnitId` vì không cần trả về
        delete result.productUnitId;
      }
